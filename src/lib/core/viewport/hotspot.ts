...
function f32swap(a: Float32Array, b: Float32Array) {
	for (let i=0, t: number; i<a.length; i++) {
		t = a[i];
		a[i] = b[i];
		b[i] = t;
	}
...
export class Mat3x2 {
	...
	constructor(v: Float32Array = new Float32Array(6)) {
...
export class RectFitter {
	...
	FitRectToSurface(
		...
		const PushFit = (rect_idx: number, rotated: boolean): RectFitResult => {
			fit_results.push(new RectFitResult(rect_idx, rotated));
			return fit_results.at(-1)!;
		};
	...
	GetFinalTransform(
		...
		const offset_x = rect.mins.x + inset;
		const offset_y = rect.mins.y + inset;
